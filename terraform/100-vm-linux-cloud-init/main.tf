...
resource "vcd_vm" "vm" {
  ...
  guest_properties = {
    "user-data" = base64encode(templatefile("cloud-config.yaml", { hostname = var.vm_name, localadmin = var.vm_localadmin_username, localpass = var.vm_localadmin_passwd, sshauthorizedkey = var.vm_ssh_authorized_key }))
  }
}